    ...
    public static Specification<CarListing> fromFilter(ListingFilterRequest filter, Location locationEntity) {
        return (root, query, criteriaBuilder) -> {
            ...
            if (StringUtils.hasText(filter.getBrand())) {
                predicates.add(criteriaBuilder.equal(criteriaBuilder.lower(root.get("brand")), filter.getBrand().toLowerCase()));
            }
            if (StringUtils.hasText(filter.getModel())) {
                predicates.add(criteriaBuilder.equal(criteriaBuilder.lower(root.get("model")), filter.getModel().toLowerCase()));
            }
